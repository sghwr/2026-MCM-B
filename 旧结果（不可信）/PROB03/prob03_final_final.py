...
class LunarWaterARMA:
    ...
    def visualize_cost_analysis(self, supply_schedule, cost_breakdown, save_path='yearly_cost_analysis.png'):
        ...
        cbar.ax.set_yticklabels(['Elevator', 'Rocket'])
        
        # Add overall statistics
        stats_text = f"""
        Overall Statistics:
        Total Cost: {cost_breakdown['total_cost']/1e6:.2f} M$
        Average Daily Cost: {cost_breakdown['avg_daily_cost']/1e4:.2f} ×10⁴$
        Average Daily Supply: {cost_breakdown['avg_supply']:.1f} tons
        Supply Standard Deviation: {cost_breakdown['supply_std']:.1f} tons
        Rocket Usage Days: {cost_breakdown['rocket_days']} days
        Elevator Usage Days: {cost_breakdown['elevator_days']} days
        """
        

        
        # Adjust layout and save
        plt.suptitle('Lunar Base One-Year Supply Cost Analysis', fontsize=16, fontweight='bold', y=0.98)
        plt.tight_layout()
